...
public interface IParkingGateService
{
    ...
    Task<CameraCaptureDto> AddCapture(Guid gateId, CreateCameraCaptureDto dto, string userId);
    
    Task<PagedResult<CameraCaptureDto>?> GetCaptures(Guid gateId, int page, int pageSize);
    
    Task DeleteCapture(Guid gateId, Guid captureId, string userId, bool isAdmin);
